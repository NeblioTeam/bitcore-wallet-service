...
var NTP1 = require('./blockchainexplorers/ntp1');
  ...
  async.series([
    ...
    function(next) {
      ...
      self._getUtxos(addressStrs, function(err, utxos) {
        ...
        for (var u in utxoIndex) {
          var x = Bitcore.Script.fromString(utxoIndex[u].scriptPubKey)
          if (x.isColdStakingOut() && utxoIndex[u].confirmations < 121) {
            log.info("LOCKING COLD-IMMATURE UTXO: " + u);
            utxoIndex[u].locked = true;
          }
        }
        return next();
      });
    ...
    function(next) {
      //for (var a = 0; a < allAddresses.length; a++) {
        ntp1.getAddressInfo(allAddresses[0].address, function(err, info) {
          if (err) return cb(err);
          for (var i = 0; i < info.utxos.length; i++) {
            if (utxoIndex[info.utxos[i].txid + '|' + info.utxos[i].index]) {
              if (info.utxos[i].tokens.length > 0) {
                log.info("LOCKING TOKEN UTXO: "+info.utxos[i].txid + '|' + info.utxos[i].index);
                utxoIndex[info.utxos[i].txid + '|' + info.utxos[i].index].locked = true;
              }
            }
          }
          return next();
  ...
  self.getWallet({}, function(err, wallet) {
    ...
    self._getUtxosForCurrentWallet(null, function(err, utxos) {
      ...
      self._getFeePerKb(wallet, opts, function(err, feePerKb) {
        ...
        baseTxpFee = Math.ceil(baseTxpFee / 10000) * 10000; // round up to nearest 10000
        ...
        info.fee = Math.ceil(info.fee / 10000) * 10000; // round up to nearest 10000
