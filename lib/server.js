...
WalletService.prototype._getUtxos = function(addresses, cb) {
  ...
  bc.getUtxos(addresses, function(err, utxos) {
    ...
    for (var i = 0; i < utxos.length; i++) {
      var utxo_matched = false;
  	  ntp1.getAddressInfo(utxos[i].address, function(err, info) {
        if (err) return cb(err);
  	    for (var u = 0; u < info.utxos.length; u++) {
  	      if (utxos[i].txid == info.utxos[u].txid && utxos[i].vout == info.utxos[u].index) {
            utxo_matched = true;
            break;
  	      }
        }
        if (!utxo_matched) {
      	  filtered.push(utxos[i]);
        }
      });
    }
